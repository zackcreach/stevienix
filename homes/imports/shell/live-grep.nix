{ ripgrep, bat, fzf, writeShellScriptBin }:

writeShellScriptBin "live-grep" ''
  rm -f /tmp/rg-fzf-{r,f}
  RG_PREFIX="${ripgrep}/bin/rg -l --color=always --smart-case "
  INITIAL_QUERY="''${*:-}"
  : | ${fzf}/bin/fzf --ansi --disabled --query "$INITIAL_QUERY" \
      ...
      --preview "${bat}/bin/bat --color=always --style=grid,numbers '{1}' | ${ripgrep}/bin/rg --pretty --no-line-number --ignore-case --context 20 {q}" \
